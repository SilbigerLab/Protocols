...
Protocol for set-up and take-down of the Oxy-4 and its probes, as well as how to use the PreSens Measurement Studio 2 to log and export data from the Oxy-4.  If you don't already have the PreSens program on your PC computer, download the software on the disk provided.
...
- [**Shutdown, Cleaning, and Storage of Oxy-4**](#Shutdown)
- [**Troubleshooting**](#Troubleshooting)  
- [**References**](#references)  


<a name="Oxy4"></a> **Oxy-4 Setup**

Be very delicate with the oxygen probes. Leave the cap on the end when not in use and do not scratch the tip.  The oxygen cord is a fiber optic cable and very fragile, it should never be coiled too tightly or bent, or the sensor will break.

1.  Put the presens in a dry area, works best on an elevated surface behind the respo stands. 
2.  Plug in the oxygen and temp probes into the corresponding ports, these should be programmed into the computer already. Temp probes donʻt matter as much as they donʻt have a calibration step.
2.  Insert each numbered Temp probe into the corresponding Temp port (lower ports) on the Oxy-4 by aligning the red dots and pushing gently until the dots meet.
3.	Remove the **black caps on the Oxy-4 Oxygen ports**, and the **red caps on the O2 probes** and place all caps in the bag labeled “Oxy-4 ST Caps”.
4.	Insert each numbered O2 probe into the corresponding O2 port (upper ports) on the Oxy-4 by aligning the L shape into channel press in, 2 dots on either side have to line up, push in and turn the probe clockwise to catch the pin.
5.	Leave the black cap on the O2 probe until you’re ready to use the probe, then place these caps in the bag labeled “Oxygen Sensor Caps”.
6.  Unravel the probe wire to make sure you have enough to go into the chamber, O2 and Temp probes should be abou the same length. 
6.	Attach the power supply to the lower port on the back of the Oxy-4 by inserting the connector and screwing until finger tight.
7.	Plug the USB connector cord into your computer and the other end into the Oxy-4, lights should be red on the Oxy4. 
8.  Open Presens app on computer
...
2. 	On the left sidebar, select "OXY-4 ST, O2" (The 4 available channels of each Presen should show up in a drop down on the sidebar).
3.	In the Measurements tab, click "New" and type in a useful data file name for each measurement of each recording period.  If you are using all 12 sets of O2 and Temp, you will need to create and title 12 measurements per recording run.  Give unique, descriptive names to help manage and organize your data.
4.	Once all your new measurements are created, you need to assign them to each channel on the Oxy-4.  In the Single Channel View on top, click Measurement and the channel that you want (must select both), click assign to.   Do this for all 12 channels with your 12 unique measurement files.
5.	When you click each Channel on the sidebar, the "Measurement" description should now show your assigned data file name, and "Sensor" should show the appropriate corresponding O2 and Temp probes (Ex. Channel 1 should show "Sensor: OXY_CH1").   If any Channels do not have the correct probe number assigned, you must recalibrate the sensor using the provided detail sheet from the probe and by following the Probe Calibration steps, detailed below. Click "rename" to edit names.
6.  You must test the salinity and temperature in the water first, go to live view and click on each channel, assign salinity at bottom. 
...
1.  Each Channel must be directed to start recording individually.  In the Live View tab, select your first Channel on the sidebar and click “Start”.  Select each subsequent Channel and click "Start" until all Channels are recording data.  You can view the live O2 readings, current temperature, and pressure for each Channel by selecting the Channel and viewing the graph or recorded parameters to the right of the graph.
    1. You should see the Oxygen increasing if the light is on and decreasing when the light is off. 
    1. Your blanks should not be moving much from the middle. 
    1. Write down time you start.
    1. Diagnose issues that you might see while looking at the data graphs (i.e flick the O2 probes, check for bubbles)
      1. Record new start time if you make any adjustments.
2.	Sensors will record every 2 second until you stop the program.
3.	To stop recording, click on the same first Channel you selected when starting, and in the Live View tab, click “Stop”.  Select each subsequent Channel and click "Stop" until all Channels have stopped recording.
...
<a name="Shutdown"></a> **Shutdown, Cleaning, and Storage of Oxy-4**

1.	Close out of the program on your computer.  Detach the power and USB cables from the Oxy-4 and store cables in their labeled bags.
1.	Disconnect the Temp and O2 probes from the Oxy-4 ports and place the **black caps on the Oxy-4 O2 ports** and the **red caps on the O2 probe ends**.
...
**If PreSens Measurement Studio 2 crashes after plugging in the Oxy 4, or PreSens shows this error message after plugging in the Oxy4 first: "Error while loading the sensors.  The application will now terminate."**  
...
- If a connection still cannot be made, update the PreSens Measurement 2 software [here](https://www.presens.de/support-services/download-center/software.html)
  ...
  - Open PreSens on your computer and plug in the Oxy4.  The sensors should now display on the left sidebar
...
* [Oxygen Dipping Probe Manual](https://github.com/SilbigerLab/Protocols/blob/master/Physiological_Parameter_Protocols/Protocols/Respirometry_Protocol/O2%20Dipping%20Probe%20Manual.pdf) 
